...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        string ingredient = "?exportSheet=Ingredient";
        string ingredientCombination = "?exportSheet=IngredientCombination";
        using HttpClient client = new();

        try
        {
            string json = await client.GetStringAsync(url + ingredient);

            Console.WriteLine("=== RESPONSE ===");
            Console.WriteLine(json);

            json = await client.GetStringAsync(url + ingredientCombination);
